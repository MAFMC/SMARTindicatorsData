...
Welcome to this google doc draft SMART indicator report for Atlantic MultiDecadal Oscillation Index. This document requires hand editing to fill some or all sections. This workflow is inspired by the [Ecosystem Context for Stock Advice](https://github.com/NOAA-EDAB/ECSA/tree/master?tab=readme-ov-file#developing-an-ecsa-project) product workflow. You are looking at an intermediate draft that still needs some work.  

Do:

* Ensure google docs automated "use smart quotes" and "enable markdown" are *not* checked: change this in the Tools-->Preferences menu
* Replace the text in [brackets] within sections beginning and ending with section begin and end markers inside double curly-bracket "mustaches". Anything in between the section markers will be harvested for the final report. Replace the text describing content for the section with the indicator specific content.
* Remove the [brackets], but leave all section markers and mustaches alone
* Complete the SMART rating for each section as instructed by replacing NA with numbers in r code snippets (e.g. r measurable.1 <- NA)
* Surround those r code snippets with ``, *leaving no spaces* as in `r measureable.1 <- 0`
* Include figures and develop brief text on how the patterns in the data might be informative . 
...
* Before reading back into an rmd, *all suggested changes must be accepted*
