...
# code by Andy Beet from ecodata catalog https://github.com/NOAA-EDAB/catalog/blob/master/R/make_rmd.R
    ...
    if (length(functionArgs) == 2 | ((length(functionArgs) == 3) & (any(c("year","scale") %in% functionArgs)))) {
      ...
      if (any(c("all","na") %in% tolower(epus)) | is.null(epus)) {
        # plot just one map
        #cat(paste0("```{r plot_",dataset,"MAB}"),append=T,fill=T,file=con)
        #cat("# Plot indicator",append=T,fill=T,file=con)
        if(isn) {
          #cat(paste0("ggplotObject <- ecodata::plot_",dataset,"(report='MidAtlantic',n=",n,")"),append=T,fill=T,file=con)
          eval(parse(text=paste0("ecodata::plot_",dataset,"(report='MidAtlantic',n=",n,")")))
        } else {
          #cat(paste0("ggplotObject <- ecodata::plot_",dataset,"(report='MidAtlantic')"),append=T,fill=T,file=con)
          eval(parse(text=paste0("ecodata::plot_",dataset,"(report='MidAtlantic')")))
        }
        ...
        # plot NewEngland Report
          ...
          cat("",append=T,fill=T,file=con) # add space
          cat(paste0("```{r plot_",dataset,"NE}"),append=T,fill=T,file=con)
          cat("# Plot indicator",append=T,fill=T,file=con)
          if(isn) {
            cat(paste0("ggplotObject <- ecodata::plot_",dataset,"(report='NewEngland',n=",n,")"),append=T,fill=T,file=con)
          } else {
            cat(paste0("ggplotObject <- ecodata::plot_",dataset,"(report='NewEngland')"),append=T,fill=T,file=con)
          }       
      ...
      cat("",append=T,fill=T,file=con) # add space
      cat(paste0("```{r plot_",dataset,"NEGB}"),append=T,fill=T,file=con)
      cat("# Plot indicator",append=T,fill=T,file=con)
      
      if(isn) {
        cat(paste0("ggplotObject <- ecodata::plot_",dataset,"(report='NewEngland',EPU='GB',n=",n,")"),append=T,fill=T,file=con)
      } else {
        cat(paste0("ggplotObject <- ecodata::plot_",dataset,"(report='NewEngland',EPU='GB')"),append=T,fill=T,file=con)
      } 
      ...
      cat("",append=T,fill=T,file=con) # add space
      cat(paste0("```{r plot_",dataset,"NEGOM}"),append=T,fill=T,file=con)
      cat("# Plot indicator",append=T,fill=T,file=con)
      
      if(isn) {
        cat(paste0("ggplotObject <- ecodata::plot_",dataset,"(report='NewEngland',EPU='GOM',n=",n,")"),append=T,fill=T,file=con)
      } else {
        cat(paste0("ggplotObject <- ecodata::plot_",dataset,"(report='NewEngland',EPU='GOM')"),append=T,fill=T,file=con)
      } 
    ...
    if (length(at) == 3 & ("varName" %in% names(at)) ){
      ...
      for (arep in at$report) {
        ...
        for (avar in at$varName) {
          ...
          cat(paste0("```{r plot_",dataset,arep,avarws,"}"),append=T,fill=T,file=con)
          cat("# Plot indicator",append=T,fill=T,file=con)
          
          if(isn) {
            cat(paste0("ggplotObject <- ecodata::plot_",dataset,"(report= '",arep,"', varName= '",avar,"',n=",n,")"),append=T,fill=T,file=con)
          } else {
            cat(paste0("ggplotObject <- ecodata::plot_",dataset,"(report= '",arep,"', varName= '",avar,"')"),append=T,fill=T,file=con)
          }   
    ...
    if (length(at) == 3 & ("plottype" %in% names(at)) ){
      ...
      for (arep in at$report) {
        ...
        for (avar in at$plottype) {
          ...
          cat("# Plot indicator",append=T,fill=T,file=con)
          if(isn) {
            cat(paste0("ggplotObject <- ecodata::plot_",dataset,"(report= '",arep,"', plottype= '",avar,"',n=",n,")"),append=T,fill=T,file=con)
          } else {
            cat(paste0("ggplotObject <- ecodata::plot_",dataset,"(report= '",arep,"', plottype= '",avar,"')"),append=T,fill=T,file=con)
          }   
      ...
      for (arep in at$report) {
        ...
        for (avar in at$varName) {
          ...
          for (aval in newVals) {
            avarws <- gsub("\\s", "", avar)  # remov whitespace from name
            # create varName plots
            cat(paste0("```{r plot_",dataset,arep,avarws,aval,"}"),append=T,fill=T,file=con)
            cat("# Plot indicator",append=T,fill=T,file=con)
            
            if(isn) {
              cat(paste0("ggplotObject <- ecodata::plot_",dataset,"(report= '",arep,"', varName= '",avar,"' ,",newV,"= '",aval,"',n=",n,")"),append=T,fill=T,file=con)
            } else {
              cat(paste0("ggplotObject <- ecodata::plot_",dataset,"(report= '",arep,"', varName= '",avar,"' ,",newV,"= '",aval,"')"),append=T,fill=T,file=con)
            }   
          ...
          for (avar in at$varName) {
            for (atype in at$plottype) {
              ...
              if (any(tolower(functionArgs) == "epu")) {
                ...
                for (aepu in EPUs) {
                  cat("",append=T,fill=T,file=con) # add space
                  cat(paste0("```{r plot_",dataset,arep,avarws,atype,aepu,"}"),append=T,fill=T,file=con)
                  cat("# Plot indicator",append=T,fill=T,file=con)
                  
                  if(isn) {
                    cat(paste0("ggplotObject <- ecodata::plot_",dataset,"(report= '",arep,"', EPU = '",aepu,"', varName= '",avar,"', plottype = '",atype,"',n=",n,")"),append=T,fill=T,file=con)
                  } else {
                    cat(paste0("ggplotObject <- ecodata::plot_",dataset,"(report= '",arep,"', EPU = '",aepu,"', varName= '",avar,"', plottype = '",atype,"')"),append=T,fill=T,file=con)
                  }  
                  cat("ggplotObject",append=T,fill=T,file=con)
                  cat("```",append=T,fill=T,file=con)
                }
                ...
                cat("",append=T,fill=T,file=con) # add space
                cat(paste0("```{r plot_",dataset,arep,avarws,atype,"}"),append=T,fill=T,file=con)
                cat("# Plot indicator",append=T,fill=T,file=con)
                
                
                if(isn) {
                  cat(paste0("ggplotObject <- ecodata::plot_",dataset,"(report= '",arep,"', varName= '",avar,"', plottype = '",atype,"',n=",n,")"),append=T,fill=T,file=con)
                } else {
                  cat(paste0("ggplotObject <- ecodata::plot_",dataset,"(report= '",arep,"', varName= '",avar,"', plottype = '",atype,"')"),append=T,fill=T,file=con)
                }  
                cat("ggplotObject",append=T,fill=T,file=con)
                cat("```",append=T,fill=T,file=con)
              }
