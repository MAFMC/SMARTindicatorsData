...
Functions used are `collect_ecodata_catalog_data()` and `collect_ecodata_indicator_stats()`. Each function defaults to collecting all available information for the full suite of SOE indicators, but can be run for individual indicators. 

The function `collect_ecodata_catalog_data()` calls `scrape_ecodata_catalog()` which pulls indicator attributes from individual Catalog fields. 
...
Functions used are `fill_indicator_temlates()` which calls `create_template_SOE()` and fills the template `SMART_template_SOE.rmd`. The `fill_indicator_templates()` function defaults to collecting all available information for the full suite of SOE indicators, but can be run for individual indicators.
