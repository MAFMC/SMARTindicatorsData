...
# this collects all available information from catalog, ecodata, and tech-doc

collect_ecodata_catalog_data()

collect_ecodata_indicator_stats()

collect_ecodata_techdoc_data()
...
# use the argument overwrite=TRUE if the template has been updated
fill_indicator_templates()

```


# Other indicators workflows

## Collect other indicator attributes

Currently done the old fashioned way, by searching for other indicators and reading the documentation. Here we will use the example of the Coastal Water Quality indicators (EPA ATTAINS) currently referenced and mapped in the Mid Atlantic Ocean Data Portal (MARCO) [here](https://portal.midatlanticocean.org/data-catalog/Water_Quality/#layer-info-epa-attains-assessment-areas4543). The MARCO portal and links to data were navigated to gather the information for each template entry.
...
# these fill one at a time; if replacing use overwrite = TRUE argument
create_template_generic("Coastal Water Quality")

# edit the gdoc according to instructions, then:
# if replacing use overwrite = TRUE argument
gdoc_to_filledrmd("Coastal Water Quality")

# this could be rendered singly to produce the report page, or using renderall() below
# to use the single function, ensure the dashes placed in the rmd title are included
render_smartind("Coastal-Water-Quality.rmd")

```

## Collect Canadian Maritimes (marea) indicator attributes

Indicators for Canada's Maritimes (and other) regions are now available online: https://github.com/MarEcosystemApproaches/marea. The R package data structure includes both metadata and indicator data. Indicators for the full North Atlantic relevant to the the Mid-Atlantic region will be included.

The function `collect_marea_indicator_stats()` calls `marea_indicator_stats()` to pull indicator attributes such as units, begin and end year, and number of years directly from the datasets. 

```{r}
# call only for desired indicators
collect_marea_indicator_stats()

```


## Create and fill marea template

This workflow uses some information from the marea indicator stats, but much still needs to be filled by hand. Therefore, the template is filled in a google doc, but with some information from the marea dataset pre-filled. The function `create_template_marea()` fills the template `SMART_template_marea.rmd` and then renders it to a google doc in the google Project folder "Projects/MAFMC Indicators/IndicatorReport/". Templates are filled for all marea indicators by default, or an individual indicator can be called in the `create_template_marea()` function. Indicator information is entered manually in the google doc(s), then pulled back into an rmarkdown document and integrated with all other indicators using the function `gdoc_to_filledrmd()`. The process is outlined in the workflow below.

```{r, eval=FALSE}

# these fill one at a time; if replacing use overwrite = TRUE argument
create_template_marea("Atlantic MultiDecadal Oscillation Index")

# edit the gdoc according to instructions, then:
# for a single file use gdoc_to_filledrmd() with the indicator name; if replacing use overwrite = TRUE argument
gdoc_to_filledrmd("Atlantic MultiDecadal Oscillation Index", overwrite = TRUE)

# to do all marea indicators from gdocs use this wrapper function
fill_marea_templates()

# this could be rendered singly to produce the report page, or using renderall() below
# to use the single function, ensure the dashes placed in the rmd title are included
render_smartind("Atlantic-MultiDecadal-Oscillation-Index.rmd")
